# Тестовое задание: Анализ и проектирование
...
- В п.2 сказано, что для удаления товара используется отдельная кнопка, минимальное количество - 1 (это же указано и в п.1)
- В п.9 говорится, что уменьшение количества до 0 приводит к удалению
- **Вывод:** Эти пункты противоречат друг другу и избыточны

**Пункты 1, 3, 4: Противоречие в установленных лимитах**
- Формулировка установленных лимитов нечеткая и может создавать конфликты
- Пример: можно добавить 5 разных товаров по 10 единиц каждого = 50 товаров, но п.4 ограничивает общее количество
- **Вывод:** В частных случаях эти пункты будут противоречивыми

**Пункт 3 и 5: Избыточная и нечеткая формулировка**
- П.5 является тавтологией, повторяя смысл п.3
- Это избыточно и может быть противоречивым, так как не несет ограничения

**Пункт 6: Не детальное описание ошибки**
- Сообщение «Лимит корзины превышен» не информативно
- Пользователь не поймет, какой именно лимит превышен:
  - Количество одного товара (п.1)
  - Число разных товаров (п.3)
  - Общее количество единиц (п.4)
- **Вывод:** Пользовательский опыт будет плохим

**Пункты 7 и 13: Противоречие ценообразования**
- П.7: цена фиксируется при добавлении и не меняется
- П.13: требуется автоматически обновлять цену в корзине при изменении в каталоге
- **Вывод:** Это взаимоисключающие требования

**Пункт 10 и 11: Неправильная классификация требований**
- П.10 и П.11 описывают не функционал корзины, а маркетинг (бизнес-правило)
- Их реализация зависит от внешних систем
- Формулировка нечеткая (какая реклама, часовые пояса, конкретное время)

**Дополнительные замечания**
- П.12 пропущен
- Отсутствуют ключевые моменты:
  - Применение промокодов/скидок
  - Итоговая сумма корзины
  - Ситуация, когда закончился товар из корзины
  - Обработка при добавлении в корзину
...
- Обновление цены для позиций в корзине пользователя происходит:
  - В момент загрузки страницы корзины
  - При любом действии с товаром в корзине (изменение количества, применение промокода)
- В процессе оформления заказа цена товара фиксируется на момент перехода на первый шаг оформления и далее не изменяется в рамках текущей сессии
...
- При действии пользователя, которое приведет к превышению любого из лимитов, система блокирует это действие и показывает всплывающее сообщение
- Сообщение должно конкретизировать причину ошибки:
  - «Достигнут лимит в Х различных товаров»
  - «Нельзя добавить более Х единиц этого товара»
  - «Вы превысили лимит в 20 товаров»

**3. Управление содержимым корзины**
- Пользователь может добавить в корзину товар из каталога (с проверкой лимитов)
- Пользователь может увеличивать или уменьшать количество для каждой позиции с помощью элементов управления «+» / «-»
- Для удаления товара используется отдельная кнопка «Удалить» для каждой позиции
- Уменьшение количества товара до 0 через элемент управления приравнивается к удалению
...
- На странице корзины для каждой позиции отображается:
  ...
  - При применении промокодов или акций: старая цена отображается зачеркнутой, новая цена с учетом скидки выделена цветом
- В отдельном блоке страницы корзины:
...
1. **Какая бизнес-модель ценообразования является приоритетной?**
   - Динамическое обновление цен или фиксация цен при добавлении в корзину?

2. **Какова бизнес-цель установления именно таких лимитов (5 товаров, 20 и 10 единиц)?**
   - Что является наивысшим приоритетом при конфликте лимитов?
   - Пример: если в корзине 4 разных товара по 6 штук = 24 штуки (нарушен общий лимит). Запрещать добавление 5-го товара или запрещать увеличение существующих?
...
4. **Можно ли получить доступ к отдельному ТЗ или требованиям рекомендаций/рекламы в корзине?**
   - Необходимо понять логику подбора рекламных товаров, правила показа, частоту обновления для корректного проектирования интеграции
...
3. **Сервис уведомлений (Notification Service):**
   ...
   - Определяет тип уведомления, получателя, шаблон
...
4. **База данных уведомлений:**
   - Хранит все уведомления с их статусами:
     - Создано
     - Отправлено
     - Доставлено
     - Прочитано
   - Сохраняет метаданные уведомлений

5. **Очередь уведомлений:**
   - Очередь для асинхронной отправки уведомлений
   - Сервис рассылки забирает задачи из этой очереди
   - Позволяет контролировать нагрузку на внешние сервисы (FCM/APNS)
   - Реализует повторные попытки при ошибках

6. **Сервис рассылки:**
   - Отвечает за отправку уведомлений через платформенные сервисы:
     - Firebase Cloud Messaging (FCM) для Android и веба
     - Apple Push Notification Service (APNS) для iOS
   - Обрабатывает ответы от этих сервисов
   - Обновляет статусы уведомлений в базе данных
